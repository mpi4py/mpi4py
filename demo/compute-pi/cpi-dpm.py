...
def view(pi, np=None, wt=None):
    ...
    if pi is not None:
        prn(f"computed pi is:  {pi:.16f}\n")
        prn(f"absolute error:  {abs(pi - PI):.16f}\n")
    if np is not None:
        prn(f"computing units: {np:d} processes\n")
    if wt is not None:
        prn(f"wall clock time: {wt:g} seconds\n")
    sys.stdout.flush()
...
def comp_pi(n, comm, root=0):
    ...
    mypi = array(mypi, "d")
    pi = array(0, "d")
    comm.Reduce([mypi, MPI.DOUBLE], [pi, MPI.DOUBLE], root=root, op=MPI.SUM)
    return pi
...
def main_parent(nprocs=1):
    ...
    icomm = MPI.COMM_WORLD.Spawn(
        command=sys.executable, args=[__file__, "child"], maxprocs=nprocs
    )
    master(icomm)
    icomm.Disconnect()
