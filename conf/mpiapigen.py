...
class Generator:
    ...
    def dump_mpiabi_h(self, fileobj, std=True):
        ...
        def abi_function(node, fallback=None):
            name = node.name
            ...
            if fallback is None:
                fallback = dedent(f"""\
                #define _pympi__{name}(...) {rvalue}
                """)
            override = dedent(f"""\
            PyMPI_LOCAL {fsign1} {fbody1}
            #define {name} {pympi1}
            """)
            return [prepare, fallback, override]
        ...
        if not std:
            ...
            for node in fstatus:
                ...
                body = f'((c) && (f) ? ({copy}, {ok}) : {err})'
                impl = dedent(f"""\
                #define _pympi__{name}{args} {body}
                """)
        ...
        if not std:
            for node in ftnconv:
                ...
                impl = dedent(f"""\
                #ifdef MPICH
                #define _pympi__{name}(arg) ({rval_m})
                #else
                #define _pympi__{name}(arg) ({rval_o})
                #endif
                """)
                for code in abi_function(node, impl):
                    fileobj.write(code)
