...
if hasattr(cygcc, "get_versions"):
    ...
    def get_versions():
        ...
        distutils.spawn.find_executable = find_executable  # ty: ignore
        versions = cygcc_get_versions()
        distutils.spawn.find_executable = find_executable_orig
        return versions

    cygcc.get_versions = get_versions  # ty: ignore
...
def configure_pyexe(exe, _config_cmd):
    ...
    if sys.platform == "darwin":
        ...
        if fwkdir and fwkdir != "no-framework":
            ...
            if fwkdir in linkforshared:  # ty: ignore[unsupported-operator]
                del libraries[:]
    #
