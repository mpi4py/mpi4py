...
class BaseTestCCOBufInter(unittest.BaseMixin):
    #
                ...
                if self.COLOR == color:
                    for root in range(size):
                        if root == rank:
                            ...
                            buf = MPI.BOTTOM
                            comm.Bcast(buf, root=MPI.PROC_NULL)
                else:
    ...
    def testReduceScatter(self):
        ...
        for array, typecode in arrayimpl.loop():
            for op in (MPI.SUM, MPI.MAX, MPI.MIN, MPI.PROD):
                ...
                rbuf = array(-1, typecode, rsize)
                comm.Reduce_scatter(sbuf.as_mpi(), rbuf.as_mpi(), rcnt, op)
                max_val = maxvalue(rbuf)
                for i, value in enumerate(rbuf):
                    if op == MPI.SUM:
                        redval = i * rsize
    ...
    def testReduceScatterBlock(self):
        ...
        for array, typecode in arrayimpl.loop():
            for op in (MPI.SUM, MPI.MAX, MPI.MIN, MPI.PROD):
                ...
                sbuf = array(list(range(size)) * rsize, typecode)
                rbuf = array(-1, typecode, rsize)
                comm.Reduce_scatter_block(sbuf.as_mpi(), rbuf.as_mpi(), op)
                max_val = maxvalue(rbuf)
                for i, value in enumerate(rbuf):
                    if op == MPI.SUM:
                        redval = i * rsize
