...
class TestExcDatatype(BaseTestCase):
    ...
    def testFreePredefined(self):
        for dtype in self.DATATYPES:
            if dtype == MPI.DATATYPE_NULL: continue
            self.assertRaisesMPI(
                self.ERR_TYPE,
                dtype.Free,
            )
...
class TestExcStatus(BaseTestCase):
    ...
    def testGetCount(self):
        ...
        self.assertRaisesMPI(
            MPI.ERR_TYPE,
            status.Get_count, MPI.DATATYPE_NULL,
        )
    ...
    def testGetElements(self):
        ...
        self.assertRaisesMPI(
            MPI.ERR_TYPE,
            status.Get_elements, MPI.DATATYPE_NULL,
        )
    ...
    def testSetElements(self):
        ...
        self.assertRaisesMPI(
            MPI.ERR_TYPE,
            status.Set_elements, MPI.DATATYPE_NULL, 0,
        )
...
class TestExcInfoNull(BaseTestCase):
    ...
    def testGet(self):
        self.assertRaisesMPI(
            [MPI.ERR_INFO, MPI.ERR_ARG],
            MPI.INFO_NULL.Get, 'key',
        )

    def testSet(self):
        self.assertRaisesMPI(
            [MPI.ERR_INFO, MPI.ERR_ARG],
            MPI.INFO_NULL.Set, 'key', 'value',
        )

    def testDelete(self):
        self.assertRaisesMPI(
            [MPI.ERR_INFO, MPI.ERR_ARG],
            MPI.INFO_NULL.Delete, 'key',
        )
...
class TestExcGroupNull(BaseTestCase):
    ...
    def testCompare(self):
        self.assertRaisesMPI(
            MPI.ERR_GROUP,
            MPI.Group.Compare, MPI.GROUP_NULL,  MPI.GROUP_NULL,
        )
        self.assertRaisesMPI(
            MPI.ERR_GROUP,
            MPI.Group.Compare, MPI.GROUP_NULL,  MPI.GROUP_EMPTY,
        )
        self.assertRaisesMPI(
            MPI.ERR_GROUP,
            MPI.Group.Compare, MPI.GROUP_EMPTY, MPI.GROUP_NULL,
        )
...
class TestExcSessionNull(BaseTestCase):
    ...
    def testGetNthPset(self):
        self.assertRaisesMPI(
            MPI.ERR_SESSION,
            MPI.SESSION_NULL.Get_nth_pset, 0,
        )
    ...
    def testGetPsetInfo(self):
        self.assertRaisesMPI(
            MPI.ERR_SESSION,
            MPI.SESSION_NULL.Get_pset_info, "mpi://SELF",
        )

    def testCreateGroup(self):
        self.assertRaisesMPI(
            MPI.ERR_SESSION,
            MPI.SESSION_NULL.Create_group, "mpi://SELF",
        )
    ...
    def testSetErrhandler(self):
        self.assertRaisesMPI(
            MPI.ERR_SESSION,
            MPI.SESSION_NULL.Set_errhandler, MPI.ERRORS_RETURN,
        )
...
class TestExcSession(BaseTestCase):
    ...
    def testGetNthPsetPos(self):
        self.assertRaisesMPI(
            MPI.ERR_ARG,
            self.SESSION.Get_nth_pset, self.SESSION.Get_num_psets(),
        )

    def testGetPsetInfo(self):
        self.assertRaisesMPI(
            MPI.ERR_ARG,
            self.SESSION.Get_pset_info, "@qerty!#$",
        )
...
class TestExcCommNull(BaseTestCase):
    ...
    def testCompare(self):
        self.assertRaisesMPI(
            self.ERR_COMM,
            MPI.Comm.Compare, MPI.COMM_NULL,  MPI.COMM_NULL,
        )
        self.assertRaisesMPI(
            self.ERR_COMM,
            MPI.Comm.Compare, MPI.COMM_SELF,  MPI.COMM_NULL,
        )
        self.assertRaisesMPI(
            self.ERR_COMM,
            MPI.Comm.Compare, MPI.COMM_WORLD, MPI.COMM_NULL,
        )
        self.assertRaisesMPI(
            self.ERR_COMM,
            MPI.Comm.Compare, MPI.COMM_NULL,  MPI.COMM_SELF,
        )
        self.assertRaisesMPI(
            self.ERR_COMM,
            MPI.Comm.Compare, MPI.COMM_NULL,  MPI.COMM_WORLD,
        )
    ...
    def testGetAttr(self):
        self.assertRaisesMPI(
            MPI.ERR_COMM,
            MPI.COMM_NULL.Get_attr, MPI.TAG_UB,
        )
    ...
    def testSetErrhandler(self):
        self.assertRaisesMPI(
            MPI.ERR_COMM,
            MPI.COMM_NULL.Set_errhandler, MPI.ERRORS_RETURN,
        )
